...
pub fn search(trees: HashMap<String, Tree>, keyword: &str, language: Language) -> Vec<String> {
    ...
    println!("HERE {}", to_sexp_with_fields(keyword_tree.root_node(), keyword.as_bytes()));
    ...
    let child = match keyword_root.child(0) {
        Some(c) => c,
        None => {
            // No child exists, return empty result
            return results;
        }
    };

    // Print the S-expression of the keyword tree
    println!("Keyword S-expression: {}", child.to_sexp());
    // Generate a query pattern from the keyword tree
    let query_pattern = format!("({} @match)", child.to_sexp());
    let query = Query::new(language, &query_pattern).expect("Invalid query");
    let mut cursor = QueryCursor::new();

    for (filename, tree) in trees {
        let root_node = tree.root_node();
        let matches = cursor.matches(&query, root_node, keyword.as_bytes());
        for m in matches {
